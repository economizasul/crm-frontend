  ...
  return (
    ...
    <div className="max-w-[1400px] mx-auto px-6 py-8">
      ...
      >
            ...
            <div className="hidden md:flex items-center gap-6">
              <div className="text-sm text-gray-600">
                <div className="font-semibold">
                    {/* 🚨 CORREÇÃO: Usa Number() para garantir que é um tipo Number antes de chamar toLocaleString */}
                    {Number(data?.productivity?.totalLeads ?? 0).toLocaleString('pt-BR')}
                </div>
                <div className="text-xs">Leads</div>
              </div>
              <div className="text-sm text-gray-600">
                <div className="font-semibold">
                    {Number(data?.productivity?.totalWonValueKW ?? 0).toLocaleString('pt-BR', { maximumFractionDigits: 0 })} kW
                </div>
       ...
       {/* Fallback messages */}
       {/* Este fallback só deve aparecer se o ReportsDashboard não estiver renderizando */}
       {!data && !loading && !error && (
